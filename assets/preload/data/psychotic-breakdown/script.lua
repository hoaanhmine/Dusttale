...
runTimer('3', 0.045, 41)--...i already know what you’re here for.
...
doTweenAlpha('speakerswitch', 'bf', 1, 0.5, 'linear')
doTweenAlpha('speakerswitch2', 'sans', 0.25, 0.5, 'linear')
runTimer('4', 0.045, 9)--Bop beep?
end
if words == 4 then
doTweenAlpha('speakerswitch', 'sans', 1, 0.5, 'linear')
doTweenAlpha('speakerswitch2', 'bf', 0.25, 0.5, 'linear')
runTimer('5', 0.045, 48)--but this time i have more XP and level than you.
cancelTimer('boploop1')
...
doTweenAlpha('speakerswitch', 'sans', 1, 0.5, 'linear')
doTweenAlpha('speakerswitch2', 'bf', 0.25, 0.5, 'linear')
runTimer('6', 0.045, 30)--let’s just get to the point.
cancelTimer('boploop1')
playAnim('sans', 'mad')
sansanim = 'mad'
runTimer('bopreset', 0.7, 1)
...
function onTimerCompleted(tag, loops, loopsLeft)
    ...
    if tag == "1" then
		wordsgoing = true
        playSound("sans", 0.5)
	setTextString('words', string.sub("heya.", 0, (loops - loopsLeft)));
	if loopsLeft == 4 then
	setTextFont("words", "comic sans.ttf")
	end
	if loopsLeft == 0 then
	...
	setTextString('words', string.sub("...i already know what you’re here for.", 0, (loops - loopsLeft)));
    if loopsLeft == 0 then
		wordsgoing = false
		inwords = true
    end
	...
	setTextString('words', string.sub("Bop beep?", 0, (loops - loopsLeft)));
    if loopsLeft == 0 then
		wordsgoing = false
		inwords = true
    end
    ...
    if tag == "5" then
		wordsgoing = true
        playSound("sans", 0.5)
	if loopsLeft == 47 then
	words = 5
	setTextFont("words", "comic sans.ttf")
	end
	setTextString('words', string.sub("but this time i have more XP and level than you.", 0, (loops - loopsLeft)));
    if loopsLeft == 0 then
		wordsgoing = false
		inwords = true
    end
	...
	setTextString('words', string.sub("let’s just get to the point.", 0, (loops - loopsLeft)));
    if loopsLeft == 0 then
		wordsgoing = false
		inwords = true
    end
